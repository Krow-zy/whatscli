...
- **Multi-profile** (`Profile` in `[general]`): per-account session DBs (`session.db` default, `session.<name>.db` otherwise). Switch via `--profile <name>` flag or `/profile <name>` in-app; listing via `/profile`. Names validated by `ValidProfileName` (`^[A-Za-z0-9_-]+$`) — never bypass this, the name is embedded in a file path. `switchProfile` (`messages/session_manager.go`) tears down client/container, calls `sm.db.Reset()`, clears `sm.currentReceiver`, and invokes `uiHandler.ResetChat()` (sync `QueueUpdateDraw`) to wipe main-UI state (`currentReceiver`, `curRegions`, textView, chat search) — any new per-account UI state must be reset there too, or it leaks across accounts.
